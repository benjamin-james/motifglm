...
#' @param jaspar JASPAR2024::JASPAR2024 or similar object
...
motif_overlap <- function(peakSet, jaspar, species="Homo sapiens", collection="CORE",
                          BSgenome=NULL,
                          width=7, cutoff=5e-05, bg="even", counts=FALSE) {
    if (is.null(BSgenome)) {
    ...
    ### get matches
    mpos = motifmatchr::matchMotifs(pwms=obj$motifs, subject=peakSet, genome=BSgenome, out="positions", p.cutoff=cutoff, w=width, bg=bg)
    ...
    return(SummarizedExperiment::SummarizedExperiment(assays=list(counts=motifMat), rowRanges=peakSet, colData=obj$motifSummary[colnames(motifMat),]))
}
